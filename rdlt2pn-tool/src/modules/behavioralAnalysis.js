...
export function simulateBehavior(pnModel, maxSteps = 1000) {
  ...
  // let splitPlaces = [];
  // let mixJoinPlaces = [];
  // for (const [id, place] of Object.entries(pnModel.places)) {
  //   if(place.splitPlace) splitPlaces.push(id);
  //   if(place.mixJoinPlace) mixJoinPlaces.push(id);
  // }

  // console.log('Split Places:',splitPlaces,'\nMix Join Places',mixJoinPlaces);
...
export function behavioralAnalysis(pnModel, maxSteps = 1000) {
  ...
  const simulationResults = simulateBehavior(pnModel, maxSteps); // Assume this exists
  
  // Evaluate termination criteria per firing sequence.
  const perSequenceResults = simulationResults.map((sequence, idx) => {
